...
A **Marketing Orchestration AI Agent Workflow** that automates marketing content production and review. The pipeline is: Topic Selection → Article Writing → Human Review → Image Generation → Xiaohongshu (小红书) copy. Human-in-the-loop decisions are managed via LangGraph's `interrupt()` mechanism.
